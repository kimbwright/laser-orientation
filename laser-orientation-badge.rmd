...
-  I notice that the final grades are all relatively similar. 

-  I wonder why the points possible is vastly different compared to the grades- why did some students only have 4 possible points? 
...
-   In my field, rows typically represent a case- usually data for one person, such as a survey response for one teacher.

What columns typically (or you think may) represent:In my field, columns typically represent variables for each case, or respondent answers to different survey questions.

-   In my field, columns typically represent variables for each case, or respondent answers to different survey questions.
...
a thought (or more): I changed the cut-off from >70 to <60, so instead of seeing only final scores greater than 70, I saw final scores less than 60.

-   I changed the cut-off from >70 to <60, so instead of seeing only final scores greater than 70, I saw final scores less than 60.
...
```{r}
d |> 
  select(student_id, Points_Possible) |> 
  arrange(desc(Points_Possible))
```
...
```{r}
ggplot(d, aes(x = Points_Possible)) +
  geom_histogram(fill = "firebrick4")
```
